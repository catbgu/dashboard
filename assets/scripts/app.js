...
function extra_profile() {

    var creators = StackMob.User.extend({ schemaName: 'Curators' });
    var myUpdate = new creators({ username: StackMob.getLoggedInUser() });
    myUpdate.save({ name: $('#fullname').val(), phone_number: $('#number').val() }, {
...
function extraresetpassword() {

    if ( $('#new-password').val() == $('#new-password-verify').val()) {
        var creators = StackMob.User.extend({ schemaName: 'Curators' });
        var user = new creators({ username: StackMob.getLoggedInUser() }); 
        user.resetPassword( $('#current-password').val(), $('#new-password').val(),{ 
            success: function(model) {
                $('#not-matching').attr("style", 'visibility: hidden');
                $('#successful-password-save').attr("style", '');
                console.debug(model);
            },
            error: function(model, response) {
                console.debug(response['error'] + model);
            }
        });
    } else {
        $('#not-matching').attr("style", 'color: red');
    }
...
jQuery(document).ready(function() { 
    ...
    items.query(q, {
      success: function(results) {
        ...
        $('#fullname').attr("value", resultsAsJSON[0].name);
        $('#username').attr("placeholder", resultsAsJSON[0].username);
        $('#number').attr("value", resultsAsJSON[0].phone_number);
      }
    });
